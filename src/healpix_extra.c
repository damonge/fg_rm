...
static void *my_malloc(size_t size)
{
  ...
  if(outptr==NULL) {
    fprintf(stderr,"healpix_extra: ran out of memory\n");
    exit(1);
  }
...
{
  fcomplex *alms=(fcomplex *)my_malloc(he_nalms(lmax)*sizeof(fcomplex));
  he_map2alm(nside,lmax,1,&map,&alms);
  
  int mm;
  for(mm=0;mm<=lmax;mm++) cls[mm]=0;

  for(mm=0;mm<=lmax;mm++) {
    int ll;
    for(ll=mm;ll<=lmax;ll++) {
      long index=he_indexlm(ll,mm,lmax);
      fcomplex alm=alms[index];
      flouble cl_ind=alm*conj(alm);
      
      if(mm==0) cls[ll]+=cl_ind;
      else cls[ll]+=2*cl_ind;
    }
  }

  for(mm=0;mm<=lmax;mm++) cls[mm]/=(2*mm+1);
}
...
void he_alm2map(int nside,int lmax,int ntrans,flouble **maps,fcomplex **alms)
{
  ...
  double time=0;
#ifdef _SPREC
  sharp_execute(SHARP_ALM2MAP,0,alms,maps,geom_info,
		alm_info,ntrans,0,&time,NULL);
#else //_SPREC
  sharp_execute(SHARP_ALM2MAP,0,alms,maps,geom_info,
		alm_info,ntrans,SHARP_DP,&time,NULL);
#endif //_SPREC
...
void he_map2alm(int nside,int lmax,int ntrans,flouble **maps,fcomplex **alms)
{
  ...
  double time=0;
#ifdef _SPREC
  sharp_execute(SHARP_MAP2ALM,0,alms,maps,geom_info,
		alm_info,ntrans,0,&time,NULL);
#else //_SPREC
  sharp_execute(SHARP_MAP2ALM,0,alms,maps,geom_info,
		alm_info,ntrans,SHARP_DP,&time,NULL);
...
float *he_read_healpix_map(char *fname,long *nside,int nfield)
{
  ...
  map=(float *)my_malloc(npix*sizeof(float));
  ...
  if(nested_in_file) {
    ...
    map_ring=(float *)my_malloc(npix*sizeof(float));
